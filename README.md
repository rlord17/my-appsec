...
`java -jar target/my-appsec-1.0-SNAPSHOT.jar`
...
docker build . -t my-appsec:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 my-appsec:latest
```
...
If the pods say `Container image "my-appsec:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
